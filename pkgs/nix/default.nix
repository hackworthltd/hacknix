{ lib, fetchurl, callPackage, storeDir ? "/nix/store", stateDir ? "/nix/var"
, confDir ? "/etc", boehmgc, stdenv, llvmPackages_6 }:
...
let
  ...
  common = { lib, stdenv, fetchpatch, perl, curl, bzip2, sqlite, openssl ? null
    ...
    , nlohmann_json, jq, libarchive, rustc, cargo, busybox-sandbox-shell
    , storeDir, stateDir, confDir, withLibseccomp ?
    ...
    let
      ...
      nix = stdenv.mkDerivation rec {
        ...
        buildInputs = [ curl openssl sqlite xz bzip2 nlohmann_json ]
          ...
          ++ lib.optional (withAWS && is20) ((aws-sdk-cpp.override {
            apis = [ "s3" "transfer" ];
        ...
        NIX_LDFLAGS = lib.optionalString (stdenv.hostPlatform.system
